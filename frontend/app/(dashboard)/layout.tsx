      ...
      <aside className={cn("fixed left-0 top-0 h-full bg-zinc-950 flex flex-col transition-all duration-300 z-40", sidebarOpen ? "w-60" : "w-16")}>
        ...
        <nav className="flex-1 px-2 py-4 space-y-1">
          {navItems.map((item) => {
            ...
            return (
              <a key={item.href} href={item.href} className={cn("flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors", isActive ? "bg-emerald-500/10 text-emerald-400" : "text-zinc-400 hover:text-white hover:bg-zinc-800")}>
